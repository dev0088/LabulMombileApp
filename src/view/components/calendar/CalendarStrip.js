...
class CalendarStrip extends Component {
  ...
  createDays = (startingDate, selectedDate = this.state.selectedDate) => {
    const { numDaysInWeek, useIsoWeekday, scrollable, minDate, maxDate, onWeekChanged } = this.props;
    ...
    for (let i = 0; i < numDays; i++) {
      ...
      if (useIsoWeekday && !scrollable) {
        ...
        date = this.setLocale(_startingDate.clone().isoWeekday(i));
      } else {
  ...
  renderHeader() {
    return (
      this.props.showMonth && (
        <View style={[styles.calendarHeaderContainer, this.props.calendarHeaderContainerStyle]}>
          <WeekSelector
    ...
    return (
      <View style={[styles.calendarContainer, { backgroundColor: this.props.calendarColor }, this.props.style]}>
        <View style={[this.props.innerStyle, { height: this.state.height }]}>
          {this.props.showDate && this.props.calendarHeaderPosition === 'above' && this.renderHeader()}

          <View style={styles.datesStrip}>
            <View onLayout={this.onLayout} style={styles.calendarDates}>
              {this.props.showDate ? this.renderWeekView(this.state.days) : this.renderHeader()}
            </View>
          </View>

          {this.props.showDate && this.props.calendarHeaderPosition === 'below' && this.renderHeader()}
        </View>
      </View>
    );
  }
